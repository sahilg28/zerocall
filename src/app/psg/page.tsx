...
import { isMuted as _isMuted, addPoints } from '@/lib/store';
  ...
  const finishShot = useCallback(() => {
    ...
    setTimeout(() => {
      if (round >= totalRounds) {
        ...
        let ptsToAdd = 15;
        const questReward = completeQuest('play_penalty');
        if (questReward > 0) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex flex-col overflow-hidden">
      ...
      <div className={`absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-4 py-3 bg-gradient-to-b from-black/85 to-transparent ${phase === 'intro' ? 'hidden' : ''}`}>
        ...
        <div className="font-pixel text-[10px] text-[var(--neon-cyan)] tracking-widest">PENALTY SHOOTER GAME</div>
